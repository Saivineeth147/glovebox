...
class AnthropicLLM:
    ...
    def turn(self, system: str, messages: list[dict[str, Any]], tools: list[dict[str, Any]]) -> Turn:
        ...
        with self._client.beta.messages.stream(
            ...
            fallbacks="default",  # type: ignore[call-overload]
        ) as stream:
        ...
        return Turn(content=content, stop_reason=msg.stop_reason or "end_turn", usage=usage, model=msg.model)
...
class ScriptedLLM:
    ...
    def __init__(self, script: list[ScriptStep], observation: Callable[[], Observation | None]) -> None:
    ...
    def turn(self, system: str, messages: list[dict[str, Any]], tools: list[dict[str, Any]]) -> Turn:
        if self._i >= len(self._script):
            return Turn(content=[{"type": "text", "text": "script exhausted"}], stop_reason="end_turn")
        ...
        block = {"type": "tool_use", "id": f"toolu_{uuid.uuid4().hex[:12]}", "name": step["tool"], "input": inp}
        text = step.get("say", f"(scripted) {step['tool']}")
        return Turn(content=[{"type": "text", "text": text}, block], stop_reason="tool_use", model="scripted")
