...
def test_redactor_registered_secret_and_patterns() -> None:
    ...
    out = r.text(
        "login pw s3cret-pass token sk-ant-abcdefghijklmnop ssn 123-45-6789 card 4111 1111 1111 1111"
    )
    assert (
        "s3cret-pass" not in out
        and "sk-ant-abc" not in out
        and "123-45-6789" not in out
        and "4111" not in out
    )
...
def test_guardrails_risk_matrix() -> None:
    ...
    assert (
        g.check_action(ActionKind.CLICK, RiskClass.IRREVERSIBLE, attended=True).verdict
        == Verdict.CONFIRM
    )
    assert (
        g.check_action(ActionKind.CLICK, RiskClass.IRREVERSIBLE, attended=False).verdict
        == Verdict.BLOCK
    )
    assert (
        Guardrails(_policy(irreversible_policy="block"))
        .check_action(ActionKind.CLICK, RiskClass.IRREVERSIBLE, attended=True)
        .verdict
        == Verdict.BLOCK
    )
    assert (
        Guardrails(_policy(allowed_actions=[ActionKind.CLICK]))
        .check_action(ActionKind.FILL, RiskClass.READ, attended=True)
        .verdict
        == Verdict.BLOCK
    )
...
def test_validate_inputs_and_templates() -> None:
    ...
    cap = _cap(
        inputs=[
            ...
            Parameter(
                name="limit", type=ParamType.INTEGER, description="", required=False, default=5
            ),
        ]
    )
