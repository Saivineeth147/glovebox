...
def el(ref, role, name, *, text=None, label=None, name_attr=None, x=0.0, y=0.0, interactive=True, tag="input",
       table=None, row=-1, col=-1):
    return Element(ref=ref, frame=["main"], role=role, name=name, text=text or name, label=label, name_attr=name_attr,
                   tag=tag, css=f"#{ref}", bbox=(x, y, 50, 20), nbox=(x / 1000, y / 1000), interactive=interactive,
                   table=table, row=row, col=col)


def test_describe_orders_strategies_and_resolve_prefers_role_name():
    els = [el("e1", "cell", "Member No.", interactive=False, tag="td", x=0, y=100),
    ...
    assert [s.kind for s in t.strategies] == ["role_name", "label", "name_attr", "near_text", "css", "bbox"]
...
def test_resolve_falls_through_when_label_renamed_across_tenant():
    t = Target(description="member input", frame=["main"], strategies=[
        TargetStrategy(kind="role_name", value={"role": "textbox", "name": "Member No."}, robustness=""),
        TargetStrategy(kind="name_attr", value={"name": "member_no", "tag": "input"}, robustness=""),
    ])
...
def test_ambiguity_is_an_error_not_a_guess():
    t = Target(description="ok button", frame=["main"], strategies=[
...
def test_table_cell_strategy_is_value_independent():
    ...
    cells = [
        ...
        el("h2", "cell", "Current Balance", interactive=False, tag="td", table=tbl, row=0, col=1, x=100, y=0),
        el("r1", "cell", "S01", interactive=False, tag="td", table=tbl, row=1, col=0, x=0, y=20),
        el("v1", "cell", "$10.00", interactive=False, tag="td", table=tbl, row=1, col=1, x=100, y=20),
    ]
    t = locators.describe(cells[3], cells)
    assert t.strategies[0].kind == "table_cell"
    assert t.strategies[0].value == {"row_anchor": "S01", "column_header": "Current Balance", "table": tbl}
    later = [c if c.ref != "v1" else el("v1", "cell", "$99.50", interactive=False, tag="td", table=tbl, row=1, col=1, x=100, y=20)
             for c in cells]
